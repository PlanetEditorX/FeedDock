...
class TestOutbound(unittest.TestCase):
    def test_host_matches_rule(self):
        cases = [
            # Empty inputs
            ("", "", False),
            (None, None, False),
        ...
        for host, rule, expected in cases:
            with self.subTest(host=host, rule=rule):
                self.assertEqual(_host_matches_rule(host, rule), expected)
    ...
    def test_should_bypass_proxy(self):
        ...
        for url, no_proxy, expected in cases:
            with self.subTest(url=url, no_proxy=no_proxy):
                config = ProxyConfig(
                    enabled=True,
                ...
                self.assertEqual(should_bypass_proxy(url, config), expected)
