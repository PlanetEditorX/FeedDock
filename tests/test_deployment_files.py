...
class DeploymentFileTests(unittest.TestCase):
    ...
    def test_refresh_actions_initial_refresh_and_log_copy(self) -> None:
        ...
        self.assertIn("previous?.last_error && rssChanged", script)
        self.assertIn("RSS 已更新，异常订阅已立即刷新一次", script)
        self.assertIn("saveRssOnly.textContent = sub.last_error ? '保存 RSS 并立即检查' : '仅保存 RSS'", script)
    ...
    def test_metadata_naming_and_local_scraping_ui_are_wired(self) -> None:
        ...
        self.assertIn('reviewManualOption', index)
        self.assertIn('确认手动信息并启动', script)
        self.assertIn('/trial/start-manual', script)
        self.assertIn('3 秒后自动关闭', script)
    ...
    def test_notification_and_subscription_monitoring_are_wired_end_to_end(self) -> None:
        ...
        self.assertIn('name="bark_encryption_algorithm"', index)
        self.assertIn('name="notification_bark_encryption_key"', index)
        self.assertIn('data-stat-target="enabled"', index)
        self.assertIn('data-stat-target="queued"', index)
        ...
        self.assertIn("async function openDashboardStat", script)
        self.assertIn("showAppView('downloads')", script)
        self.assertIn("state.value = 'enabled'", script)
        ...
        self.assertIn('bark_encryption_enabled', notification_module)
        self.assertIn('bark_encryption_algorithm', notification_module)
