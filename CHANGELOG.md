...
- Bark 通知新增与官方 App 一致的 AES128/AES192/AES256、CBC/ECB/GCM、pkcs7/noPadding 手动选择，并支持加密 Key 的遮蔽、保留、清除和备份控制。
- CBC/GCM 推送为每条消息生成独立随机 IV，仅向 Bark 服务发送密文、IV 和 Device Key。
- 首页“启用订阅、已推送、已跳过、错误”统计卡改为可点击入口，自动进入对应页面并应用状态筛选。

### 试看迁移空目录清理

- 试看视频迁移到正式媒体目录后，同步删除原位置留下的空目录。
- qBittorrent 在线迁移和本地文件兜底迁移均执行相同清理，并严格限制在媒体根目录内。

### 添加订阅搜索显示隐藏番剧

- 季度目录默认浏览仍会过滤已隐藏番剧。
- 输入标题主动搜索时忽略隐藏过滤，已隐藏番剧仍可显示、打开详情并执行订阅操作。
- 搜索结果中的隐藏番剧统一显示“已隐藏”，不再附加“搜索可见”说明。

### 试看启动后立即刷新订阅

- 试看番剧确认元数据并转为正式订阅后，立即安排一次针对该订阅的 RSS 刷新。
- 手动填写元数据启动试看时使用相同的即时刷新流程。
- 系统日志使用“试看启动刷新”标识该次检查，前端提示同步显示已开始刷新。

### 顶部菜单箭头渲染修复

- 顶部下拉菜单改用独立 SVG chevron，不再通过边框旋转模拟箭头。
- 统一箭头的尺寸、线宽、文字间距和垂直对齐。
- 菜单关闭时箭头向下，展开时围绕中心旋转 180° 向上，避免不同菜单出现偏高、贴字或形状不一致。
...
- “全部试看”下载目录按番剧拆分为 `试看/番剧名/`，并自动迁移旧的共享 `试看/` 路径配置。
