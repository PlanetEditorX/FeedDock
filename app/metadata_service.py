...
class MetadataService:
    ...
    def search(
        ...
        *,
        ...
        for level, search_query in enumerate(queries):
            ...
            if provider == "tmdb":
                candidates = self._search_tmdb(db, config, req)
            elif provider == "bangumi":
                candidates = self._search_bangumi(db, config, req)
            else:
                candidates = self._search_anilist(db, req)
    ...
    def _search_tmdb(
        self, db: Session, config: MetadataConfig, req: MetadataSearchQuery
    ) -> list[MetadataCandidate]:
        if not config.tmdb_read_access_token:
            raise ValueError("尚未配置 TMDB API Key 或 Read Access Token")
        kind = "movie" if req.media_type == "movie" else "tv"
        headers, auth_params = self._tmdb_auth(config.tmdb_read_access_token)
        params: dict[str, Any] = {
            "query": req.query, "language": config.language, "include_adult": "false", "page": 1,
            **auth_params,
        }
        if req.year:
            params["primary_release_year" if kind == "movie" else "year"] = req.year
        ...
        for item in (payload.get("results") or [])[:req.limit]:
    ...
    def _search_bangumi(
        ...
        body = {"keyword": req.query, "sort": "match", "filter": {"type": [2], "nsfw": False}}
        ...
        with external_client(url, db=db, timeout=self.timeout) as client:
            ...
            payload = response.json()
        candidates: list[MetadataCandidate] = []
        for item in (payload.get("data") or [])[:req.limit]:
            date_value = str(item.get("date") or "")
            item_year = int(date_value[:4]) if len(date_value) >= 4 and date_value[:4].isdigit() else 0
            if req.year and item_year and abs(item_year - req.year) > 2:
                continue
    ...
    def _search_anilist(self, db: Session, req: MetadataSearchQuery) -> list[MetadataCandidate]:
        query = """
        ...
        """
        data = self._anilist_request(db, query, {"search": req.query, "perPage": min(req.limit, 20)})
        candidates: list[MetadataCandidate] = []
