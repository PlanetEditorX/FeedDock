...
def load_notification_config(db: Session) -> NotificationConfig:
    ...
    return NotificationConfig(
        ...
        bark_server_url=(rows.get("notification_bark_server_url", "https://api.day.app").strip().rstrip("/") or "https://api.day.app"),
...
def save_notification_config(
    ...
    *,
    ...
    headers_raw = json.dumps({str(key): str(value) for key, value in headers.items()}, ensure_ascii=False, separators=(",", ":"))
    ...
    existing = {
        row.key: row for row in db.scalars(select(AppSetting).where(AppSetting.key.in_(_NOTIFICATION_KEYS)))
    }
